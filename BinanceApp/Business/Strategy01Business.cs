...
{
    public class Strategy01Business
    {
 ...
 //       private List<IBinanceKline> candels;
        ...
        public Strategy01Business(string _coinName, KlineInterval _interval, int _percentageMargin)
        {
        ...
        internal void DataReady(BinanceModel binanceModel)
        {
            ...
            switch (interval)
            {
                case KlineInterval.OneMinute:
                    ...
                    samples = binanceModel.Candels_1min.Skip(binanceModel.Candels_1min.Count() - 150).Take(150).ToList();
                    break;
                case KlineInterval.FiveMinutes:
                    if (binanceModel.Candels_5min == null || binanceModel.Candels_5min.Any() == false) return;

                    samples = binanceModel.Candels_5min.Skip(binanceModel.Candels_5min.Count() - 150).Take(150).ToList();
                    break;
                case KlineInterval.FifteenMinutes:
                    if (binanceModel.Candels_15min == null || binanceModel.Candels_15min.Any() == false) return;

                    samples = binanceModel.Candels_15min.Skip(binanceModel.Candels_15min.Count() - 150).Take(150).ToList();
                    break;
            }
            if (samples == null || samples.Count() == 0)
                return;
            macdResult = BinanceHelper.MacdCalculation(binanceModel.Candels_15min.Skip(binanceModel.Candels_15min.Count() - 150).Take(150).ToList()
